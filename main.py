...
def main(args):
    ...
    if args.verbose:
        ...
        logging.info(
            f"Random forest SMAPE score: {smape(y_val, rf.predict(X_val))}")
    ...
    if args.verbose:
        ...
        logging.info(
            f"XGBoost SMAPE score: {smape(y_val, xgb.predict(X_val))}")
    ...
    if args.verbose:
        ...
        logging.info(
            f"LightGBM SMAPE score: {smape(y_val, lgb.predict(X_val))}")
    ...
    if args.verbose:
        ...
        logging.info(
            f"CatBoost SMAPE score: {smape(y_val, cat.predict(X_val))}")
    ...
    #     logging.info(
    #         f"Neural Network SMAPE score: {r2_score(y_val, nn.predict(X_val.astype(float)))}")

    # lstm = models.apply_lstm(X, y)
    # if args.verbose:
    #     logging.info("LSTM applied!")
    #     logging.info(
    #         f"LSTM SMAPE score: {r2_score(y_val, lstm.predict(X_val.astype(float)))}")
    ...
    if args.verbose:
        ...
        logging.info(
            f"Decision Tree SMAPE score: {smape(y_val, dt.predict(X_val))}")
    ...
    if args.verbose:
        ...
        logging.info(
            f"Gradient Boosting SMAPE score: {smape(y_val, gb.predict(X_val))}")
